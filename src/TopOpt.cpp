...
TopOpt::TopOpt(int nelx, int nely, tfloat volfrac, tfloat penal, tfloat rmin, tfloat maxChange)
	...
	mPassiveRadius(0.0)
{
	x = std::make_shared<Matrix>(nely, nelx);
	xNew = std::make_shared<Matrix>(nely, nelx);

	dc = std::make_shared<Matrix>(nely, nelx);
	dcNew = std::make_shared<Matrix>(nely, nelx);


	F.resize(2 * (nelx + 1)*(nely + 1));
	U.resize(2 * (nelx + 1)*(nely + 1));
	initKE();
	initFreeDofs();
...
bool TopOpt::step(tfloat& compliance, tfloat& volume, tfloat& change, bool verbose /*= false*/)
{
	...
	// print results
	if (verbose) {
		std::cout << "compliance: " << std::to_string(compliance)
			<< std::endl << "volume: " << std::to_string(volume)
...
void TopOpt::exportResultMatlab(Matrix *x, int iteration)const {
	...
	printf("imwrite(1-x,'topopt_passive_%f_%i.png');", mPassiveRadius, iteration);
#ifdef RAMP    
...
void TopOpt::calculatePassive() {
	...
	radius = mPassiveRadius;
	std::cout << "Passive Radius =" << radius << std::endl;
...
void TopOpt::FEAnalysis() {
	...
	// solve KU=F <=> U = F/K
	bool solved = K.solve(F.data(), U.data(), fixeddofs);
	assert(solved);
